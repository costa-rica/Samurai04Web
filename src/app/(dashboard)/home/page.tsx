...
export default function Home() {
	...
	// ==== Chat state ====
	const [messageHistoryArray, setMessageHistoryArray] = useState<ChatMessage[]>(
		[]
	);
	...
	const handleUploadContextFile = async (
		...
		try {
			...
			if (!response.ok || (resJson && resJson.ok === false)) {
				const errorMessage =
					resJson?.error || `Upload failed with status ${response.status}`;
				throw new Error(errorMessage);
			}

			setStatus(resJson?.message || "Upload successful.");
			setFile(null);
			form.reset();
			fetchUserContextDataFiles();
		} catch (err) {
